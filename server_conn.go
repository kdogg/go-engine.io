...
// KK vvvvvvvvvvvvvvvvvvvvvv
type lockingWriteCloser struct {
	w   io.WriteCloser
	mut *sync.Mutex
}

func (l *lockingWriteCloser) Close() error {
	l.mut.Lock()
	defer l.mut.Unlock()
	return l.w.Close()
}

func (l *lockingWriteCloser) Write(p []byte) (int, error) {
	l.mut.Lock()
	defer l.mut.Unlock()
	n, err := l.w.Write(p)
	return n, err
...
func (c *serverConn) OnPacket(r *parser.PacketDecoder) {
	...
	case parser.PING:
		...
		t := c.getCurrent()
		u := c.getUpgrade()
		newWriter := t.NextWriter // KK this WriteCloser needs to aquire the lock
